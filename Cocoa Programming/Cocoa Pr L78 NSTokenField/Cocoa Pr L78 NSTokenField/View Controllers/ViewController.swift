...
class ViewController: NSViewController {
    ...
    func match(suggestion: String) -> MailToken? {
        suggestions.first { token in
            // This should really just bechecking for a valid email format
    ...
    @objc func removeAddress(_ sender: NSMenuItem) {
        ...
        guard let selectedRange = fieldEditor?.selectedRanges, var tokens = tokenField.objectValue as? [Any] else { return }
