...
class CollectionViewController: NSViewController {
    ...
    override func viewDidLoad() {
        ...
        collectionView.collectionViewLayout = /*listLayout()*/ /*gridLayout()*/ /*varyingSectionsLayout()*/ bidirectionalLayout()
    }
    ...
    private func varyingSectionsLayout() -> NSCollectionViewLayout {
        let layout = NSCollectionViewCompositionalLayout { (sectionIndex: Int, layoutEnvironment: NSCollectionLayoutEnvironment) -> NSCollectionLayoutSection in
            let columns = (sectionIndex * 2) + 1
            
            // The `group` auto-calculates the actual item width to make the requested number of `columns` fit, so this `widthDimension` will be ignored
            let itemSize = NSCollectionLayoutSize(widthDimension: .fractionalWidth(1.0), heightDimension: .fractionalHeight(1.0))
            let item = NSCollectionLayoutItem(layoutSize: itemSize)
    ...
    private func bidirectionalLayout() -> NSCollectionViewLayout {
        ...
        let containerGroup = NSCollectionLayoutGroup.horizontal(layoutSize: NSCollectionLayoutSize(widthDimension: .fractionalHeight(1.4), heightDimension: .fractionalHeight(0.5)), subitems: [leadingItem, trailingGroup])
